...
const db = {
  ...
  save: (data) => {
    ...
    try {
      localStorage.setItem('drachenboot_pwa_data_v2', JSON.stringify(data));
...
const ImprintModal = ({ onClose }) => (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
          <h2 className="text-lg font-bold text-slate-800 dark:text-white">Impressum</h2>
          <button onClick={onClose} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full transition-colors text-slate-500">
            <X size={20}/>
          </button>
        </div>
...
const DrachenbootPlaner = () => {
  ...
  const seedInitialData = () => {
      ...
      setEvents([{ id: initEventId, title: 'Erstes Training', date: new Date().toISOString().split('T')[0], type: 'training', attendance: initialPaddlers.reduce((acc, p) => ({...acc, [p.id]: 'yes'}), {}) }]);
  ...
  const activePaddlerPool = useMemo(() => {
    ...
    if (activeEventId && activeEvent) {
        pool = paddlers.filter(p => ['yes', 'maybe'].includes(activeEvent.attendance[p.id]));
    }
        ...
        for (let i = 0; i < 1500; i++) {
            ...
            for(let p of currPool) {
                ...
                Object.entries(currAss).forEach(([sid, pid]) => {
                    ...
                    const pad = paddlers.find(x => x.id===pid); if(!pad) return;
              ...
              <div className="max-w-6xl mx-auto"> 
                  <header className="mb-6 flex justify-between items-center bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 sticky top-0 z-30">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-slate-900 dark:text-white leading-none">Team Manager</h1>
                            <p className="text-xs text-slate-500 dark:text-slate-400">Kader & Termine</p>
                        </div>
                    </div>
                    <div className="flex gap-2">
                        <button onClick={() => setShowHelp(true)} className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                            <Info size={20}/>
                        </button>
                        ...
                        <button onClick={() => setView('planner')} className="px-4 py-2 bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-slate-700 rounded-lg font-medium flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      <div className="lg:col-span-1 space-y-4">
                          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800">
                              <h3 className="font-bold text-slate-700 dark:text-slate-200 mb-3 flex items-center gap-2 text-sm uppercase tracking-wide"><Calendar size={16}/> Neuer Termin</h3>
                                  ...
                                  return (
                                      <div key={evt.id} className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-md transition-shadow">
                                          <div className="flex justify-between items-start mb-2">
                                              ...
                                              <div className={`text-[10px] px-2 py-1 rounded-full font-bold uppercase ${evt.type === 'race' ? 'bg-purple-100 dark:bg-purple-900 text-purple-700 dark:text-purple-200' : 'bg-blue-50 dark:bg-blue-900 text-blue-600 dark:text-blue-200'}`}>{evt.type === 'race' ? 'Regatta' : 'Training'}</div>
                                          </div>
                      ...
                      <div className="lg:col-span-2 space-y-6">
                           <div className={`p-6 rounded-xl shadow-sm border transition-all ${editingPaddlerId ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800 ring-1 ring-orange-200 dark:ring-orange-900' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800'}`}>
                                <h3 className={`font-bold mb-4 flex items-center gap-2 text-sm uppercase tracking-wide ${editingPaddlerId ? 'text-orange-800 dark:text-orange-200' : 'text-slate-700 dark:text-slate-200'}`}>{editingPaddlerId ? <Pencil size={16}/> : <User size={16}/>} {editingPaddlerId ? 'Paddler bearbeiten' : 'Neues Mitglied'}</h3>
                                    ...
                                    {sortedPaddlers.map(p => (
                                        <div key={p.id} className={`p-3 border rounded-xl transition-all relative group ${editingPaddlerId === p.id ? 'border-orange-400 bg-orange-50 dark:bg-orange-900/20' : 'bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-750'}`}>
                                            ...
                                            <div className="absolute bottom-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => handleEditPaddler(p)} className="bg-white dark:bg-slate-700 text-slate-400 hover:text-orange-500 p-1.5 rounded-lg border dark:border-slate-600 shadow-sm" title="Bearbeiten"><Pencil size={12}/></button>
                                                <button onClick={() => triggerDelete(p.id)} className={`p-1.5 rounded-lg border shadow-sm transition-colors ${deleteConfirmId === p.id ? 'bg-red-600 text-white border-red-700' : 'bg-white dark:bg-slate-700 text-slate-400 hover:text-red-500 dark:border-slate-600'}`} title={deleteConfirmId === p.id ? "Wirklich löschen?" : "Löschen"}><Trash2 size={12}/></button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
    ...
    <div className={`min-h-screen font-sans text-slate-800 dark:text-slate-100 transition-colors duration-300 bg-slate-100 dark:bg-slate-950 p-2 md:p-4 pb-20`}>
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-4 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-3 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 sticky top-0 z-30">
          ...
          <div className="flex gap-2 items-center">
            ...
            <div className="text-center px-2"><div className="text-[10px] text-slate-400 uppercase font-bold">Besetzt</div><div className="font-bold text-sm text-blue-600 dark:text-blue-400">{stats.c} / 22</div></div>
             ...
             <button onClick={toggleDarkMode} className="p-2 rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-blue-50 dark:hover:bg-slate-700">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1 space-y-4">
            <div className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800">
              <h2 className="font-bold text-sm text-slate-700 dark:text-slate-200 uppercase tracking-wide mb-3 flex items-center gap-2"><ArrowRightLeft size={16}/> Balance & Schwerpunkt</h2>
              ...
              <div className="grid grid-cols-3 gap-2 mt-4">
                  ...
                  <button onClick={clearBoat} className={`py-3 text-sm rounded-lg border dark:border-slate-700 transition-all flex items-center justify-center gap-2 active:scale-95 ${confirmClear ? 'bg-red-500 text-white border-red-600 font-bold' : 'text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 border-red-200'}`}>{confirmClear ? <AlertCircle size={16}/> : <RotateCcw size={16}/>}</button>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 h-[600px] flex flex-col">
              <h2 className="font-bold text-sm text-slate-700 dark:text-slate-200 uppercase tracking-wide mb-3 flex items-center gap-2"><User size={16}/> Verfügbar ({activePaddlerPool.length})</h2>
                ...
                {activePaddlerPool.map(p => {
                  ...
                  return (
                    <div key={p.id} onClick={() => setSelectedPaddlerId(isSelected ? null : p.id)} className={`p-3 rounded-xl border cursor-pointer transition-all flex justify-between items-center group ${isSelected ? 'bg-blue-600 border-blue-700 text-white shadow-md transform scale-[1.02]' : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-600'} ${isAssigned ? 'opacity-40 grayscale' : ''} ${isMaybe ? 'border-yellow-300 bg-yellow-50 dark:bg-yellow-900/20 dark:border-yellow-700' : ''}`}>
                      <div>
                        <div className={`text-base font-bold ${isSelected ? 'text-white' : 'text-slate-800 dark:text-slate-200'}`}>{p.name} {isMaybe && <span className="text-xs opacity-70">(?)</span>}</div>
          ...
          <div className="lg:col-span-2 pb-10">
            <div className="bg-blue-100/30 dark:bg-blue-900/20 p-4 md:p-8 rounded-3xl border border-blue-100 dark:border-blue-800 min-h-[800px] flex justify-center items-start overflow-y-auto relative">
              ...
              <div ref={boatRef} className="relative w-[360px] flex flex-col items-center">
                ...
                <div className="relative bg-amber-50 dark:bg-amber-900 border-4 border-amber-800 dark:border-amber-950 shadow-xl w-full px-4 py-12 z-10" 
                     style={{
                         clipPath: 'polygon(50% 0%, 80% 2%, 95% 10%, 100% 45%, 100% 55%, 95% 90%, 80% 98%, 50% 100%, 20% 98%, 5% 90%, 0% 55%, 0% 45%, 5% 10%, 20% 2%)',
                         borderRadius: '40px' 
                     }}>
                    ...
                    <div className="space-y-3 pt-6 pb-6 relative z-10">
                      <div className="flex justify-center mb-8">
                         {(() => {
                            ...
                            const p = paddlers.find(x=>x.id===assignments[s.id]);
                      ...
                      {Array.from({ length: rows }).map((_, i) => {
                        ...
                        const pl = paddlers.find(x=>x.id===assignments[ls]);
                        const pr = paddlers.find(x=>x.id===assignments[rs]);
...
const BalanceBar = ({ left, right, diff }) => {
    ...
    return (
      <div className="w-full mb-3">
        <div className="flex justify-between text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 mb-1"><span>Links</span> <span>Diff: {Math.abs(diff)}</span> <span>Rechts</span></div>
...
const TrimBar = ({ front, back, diff }) => (
    <div className="w-full p-3 bg-blue-50/50 dark:bg-blue-900/30 rounded-lg border border-blue-100 dark:border-blue-800">
         <div className="flex justify-between text-xs text-blue-800 dark:text-blue-300 font-bold mb-1"><span className="flex gap-1"><ArrowUpFromLine size={12}/> Bug: {front}</span><span className="flex gap-1"><Anchor size={12}/> Heck: {back}</span></div>
...
const SeatBox = ({ seat, paddler, isLocked, isSelected, onClick, onUnassign, onLock, getSkillBadges, hideWeight, isMaybe }) => {
    const side = seat.id.includes('left') ? 'left' : seat.id.includes('right') ? 'right' : null;
    let base = 'bg-white dark:bg-slate-800 border-amber-200/50 dark:border-slate-600 hover:border-blue-400 shadow-sm', text = 'text-slate-800 dark:text-slate-200', badge = '';
    ...
    if (paddler) {
        ...
        if (ok) { base = 'bg-green-200 dark:bg-green-800 border-green-700 dark:border-green-600 shadow-md'; text = 'text-green-900 dark:text-green-100 font-bold'; } 
        else { base = 'bg-red-200 dark:bg-red-900 border-red-600 dark:border-red-700 shadow-md'; text = 'text-red-900 dark:text-red-100 font-bold'; }
    ...
    return (
        <div onClick={onClick} className={`w-24 h-14 rounded-lg border-2 flex flex-col items-center justify-center cursor-pointer transition-all relative z-10 group ${base} ${active}`}>
            {paddler ? (
                <>
                    <div className={`w-full px-1 text-center ${hideWeight ? 'flex h-full items-center justify-center' : ''}`}>
                        <span className={`leading-tight break-words ${hideWeight ? 'text-xs font-bold' : 'text-sm block truncate'} ${text}`}>
                            {paddler.name} {isMaybe && <span className="text-yellow-700 dark:text-yellow-300 font-extrabold text-xs ml-1" title="Vielleicht">?</span>}
                        </span>
                    </div>
                ...
                seat.type === 'drummer' ? <Drum size={18} className="text-slate-400 dark:text-slate-600 opacity-70"/> :
                seat.type === 'steer' ? <ShipWheel size={18} className="text-slate-400 dark:text-slate-600 opacity-70"/> :
